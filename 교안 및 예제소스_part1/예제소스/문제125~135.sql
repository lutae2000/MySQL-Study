...
delimiter //
create procedure q126(in inename varchar(20))
begin
	select *
      from emp
	where ename = inename;
...
delimiter //
create procedure q127(in in_dname varchar(14))		
begin
	select *
      from dept
    where dname = in_dname;
end //
delimiter ;
call q127('경리부'); 			
...
delimiter //
create procedure q128(in in_job varchar(8), in_hiredate date)
begin
	select *
      from emp
	where job = in_job
      and hiredate >= in_hiredate;
end //
delimiter ;
...
delimiter //
create procedure q129(in in_empno int, out out_ename varchar(20))
begin
...
select concat('1001사원번호의 이름 : ', @answer);
...
delimiter //
create procedure q130(in in_ename varchar(20))
begin
	declare message varchar(20);  -- 메시지 저장변수
    declare in_job varchar(8);  -- 직급을 저장할 변수
...
delimiter //
create procedure q131(in in_ename varchar(20))
begin
	declare message varchar(20);  -- 메시지 저장변수
    declare in_job varchar(8);  -- 직급을 저장할 변수
    ...
    select job into in_job
     ...
     case
		when in_job = '사장' or in_job = '부장' then
			set message = '이제 정년이 다 되어가는군요!';
		when in_job = '차장' or in_job = '과장' or in_job = '대리' then
			set message = '더욱 더 분발하세요';
		else
			set message = '초지일관 하세요';
	 end case;  
...
delimiter //
create procedure q135(in in_ename varchar(20))
begin
	declare in_dname varchar(14);
    
    select D.dname into in_dname
      from emp E
      inner join dept D
      on E.deptno = D.deptno
	 where E.ename = in_ename;
     
     if(in_dname = '영업부') then
