...
create table backup_emp02(
    ...
    moduser varchar(10) not null default 'root'
);
select * from emp02;
...
begin
    ...
    insert into backup_emp02 values(new.empno, new.ename, new.job,'신규',
                                    sysdate(), default);																		
end //
delimiter ;

insert into emp02 values(1, '김연아', '과장');
insert into emp02 values(2, '손연재', '대리');
select @message;
select *
  from backup_emp02;					
...
create trigger emp_updatetrg
    ...
    for each row
begin
	set @message = concat(old.ename, ' ', old.job, '님이 ', new.job,'승진하셨습니다.');
    update backup_emp02
	   set modtype = '수정'
...
create table shipmenttbl(
	num int auto_increment primary key,
    warecode varchar(10) not null,   
    shipdate datetime not null default now(),
...
begin
    ...
    -- 현 재고를 알아내는 방법
    select stock into tmp_stock
      from producttbl
	 where prodname = new.prodname;
     
	-- 재고가 음수가 되지 않게 설정
	if(tmp_stock >= new.order_count) then
		update producttbl
...
begin
	if(old.stock > new.stock) then
		insert into shipmenttbl values (null, old.prodcode, default,
             (old.stock - new.stock),((old.stock - new.stock) * old.price));
	end if;
end //
delimiter ;
insert into ordertbl values (null, '김기수','세탁기',20);				
insert into ordertbl values (null, '김기수','세탁기',120);				
insert into ordertbl values (null, '김기수','컴퓨터',41);				
...
create trigger ordertbl_deletetrg
    ...
    for each row
begin
    -- 주문을 취소(delete)를 하게 되면 당연히 재고수량이 올라가야 됨(자동으로) 
	update producttbl
       set stock = stock + old.order_count
     where prodname = old.prodname;
     
     delete from shipmenttbl
     where num = old.orderno;
end //
delimiter ;
