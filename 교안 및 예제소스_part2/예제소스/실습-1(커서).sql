...
-- 커서(cursor)는 테이블을 쿼리를 한 후, 쿼리의 결과인 행 집합을 한 행씩 처리하기
-- 위한 방식이다.
-- C언어나 자바에서 파일 입출력과 아주 유사한 개념이다.
-- 파일을 처리하기 위해서 먼저 파일을 열고, 첫 번째 데이터를 읽고 다음 데이터가 저장
-- 되어 있는 공간까지 파일포인터가 이동한다.
-- 이런씩으로 파일의 끝(EOF-end of file)까지 반복한다.
-- 마지막으로 파일 포인터를 닫는다.

-- 고객의 평균키를 구하는 것이다.단, 커서를 이용해서 말이다.
...
begin
    ...
    declare cnt int default 0;  -- 고객의 인원수(=fetch한 행의 개수)
    declare total int default 0;  -- 고객 키의 총합
    -- 플래그 변수(행의 끝인지 아닌지를 알아보는 변수)
    declare endofrow boolean default false;
    -- 아래와 같이 조회를 하면 키 값들이 출력이 될 것이다.
    -- 그리고 난 후 cursor가 그 키 값에 처음 부분에 위치하는 것이다.
    declare usercursor cursor for
					select height
				      from usertbl;
    -- 만약에 커서가 이동하면서 마지막에 도달하게 되면
    -- 더 이상 데이터를 발견 못하면 아래 핸들러가 실행이 되어서
    ...
    cursor_loop : loop
        ...
        -- 무한 루프를 빠져나가는 조건은 endofrow가 true로 되어야 한다는 것이다.
        if(endofrow) then
			leave cursor_loop;
		end if;
        
        set cnt = cnt + 1;  -- 고객수 증가
        set total = total + userheight;  -- 고객의 키를 계속 누적시킨다.
...
-- 한 가지 더 예제를 해보도록 하자
-- usertbl에 grade컬럼을 추가해서 활용해보도록 하자.
alter table usertbl
...
begin
    ...
    declare usergrade varchar(10);  -- 고객 등급을 저장할 변수
    
    -- 플래그 변수
    declare endofrow boolean default false;
       
    -- 커서를 아래 쿼리문에 선언하였다.
    -- 외부조인을 하는 이유는 하나도 구매하지 않은 고객들도 있기 때문이다.
	declare usercursor cursor for
    ...
    grade_loop : loop
        ...
        -- 조회 결과 중 hap에 따라서 고객등급(grade)를 설정한다.
         case
			when(hap >= 1500) then 
				set usergrade = '최우수고객';
			when(hap >= 1000) then
